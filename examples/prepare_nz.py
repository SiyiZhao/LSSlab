...
def main() -> None:
    ...
    source = Path("/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/loa-v1/LSScats/v2/PIP/LRG_NGC_nz.txt")
    dest = Path("data/example_nz.txt")
    prepare_nz(source, dest)

    # Optional scaling with `times`
    dest = Path("data/example_nz_times_10.txt")
